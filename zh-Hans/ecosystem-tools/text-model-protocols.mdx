...
| `deepseek-v3-2-251201` | 可用 | 需单独验收 | 可用 | 优先走 Chat Completions 或 Responses |
| `deepseek-v3.2` | 可用；依赖 streaming 的工具需单独验收 | 不建议默认使用 | 不支持 | 不建议作为生态工具默认模型 |
| `gpt-oss-120b-250805` | 可用 | 需单独验收 | 需确认权限 | 优先走 Chat Completions |
| `seed-1-6-250915` | 可用 | 需单独验收 | 可用 | 优先走 Chat Completions 或 Responses |
| `seed-1-6-flash-250715` | 可用 | 需单独验收 | 可用 | 优先走 Chat Completions 或 Responses |
| `seed-1-8-251228` | 可用 | 需单独验收 | 可用 | 优先走 Chat Completions 或 Responses |
| `seed-2-0-lite-260228` | 可用 | 需单独验收 | 可用 | 优先走 Chat Completions 或 Responses |
| `seed-2-0-mini-260215` | 可用 | 需单独验收 | 可用 | 优先走 Chat Completions 或 Responses |
| `seed-2-0-pro-260328` | 可用 | 需单独验收 | 可用 | 优先走 Chat Completions 或 Responses |
...
- `deepseek-v3.2` 不建议作为依赖 streaming 的生态工具默认模型。
- BytePlus、Seed 和 `gpt-oss-120b-250805` 的 Anthropic 工具链需要单独验收，默认优先走 Chat Completions。
- `gpt-oss-120b-250805` 使用 Responses 前需确认权限。
