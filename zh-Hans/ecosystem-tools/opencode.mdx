...
| --- | --- | --- | --- |
| 推荐文本路径 | custom provider + `@ai-sdk/openai-compatible` | `https://baze-api.powerbuyin.top/v1` | `glm-5.2` |
| MiniMax 文本路径 | custom provider + `@ai-sdk/openai-compatible` | `https://baze-api.powerbuyin.top/v1` | `MiniMax-M3` |
| Claude/Anthropic 路径 | `@ai-sdk/anthropic` custom provider | `https://baze-api.powerbuyin.top/v1` | 仅在你需要复核 `/v1/messages` 时使用 |
...
Anthropic 路径推荐优先使用 `glm-5.2`；MiniMax 场景推荐使用 `MiniMax-M3`。不要把 BytePlus、Seed、`gpt-oss-120b-250805` 或 `deepseek-v3.2` 作为 opencode Anthropic provider 的默认模型。
...
- `deepseek-v3.2` 当前不建议作为 opencode 默认模型；如必须使用，请先完成 streaming 和工具链验收。
- BytePlus、Seed 和 `gpt-oss-120b-250805` 请优先走 OpenAI-compatible provider；Anthropic provider 需单独验收。
