...
| 复核 Anthropic Messages 路径 | 仅在当前 Hermes 版本确认会请求 `/v1/messages` 后使用 `api_mode: anthropic_messages` | `https://baze-api.powerbuyin.top` | 智谱、MiniMax、Qwen 推荐模型 |
| 使用 OpenAI-compatible 优先模型 | Custom endpoint | `https://baze-api.powerbuyin.top/v1` | BytePlus、DeepSeek 等不建议走 Anthropic provider 的文本模型 |
