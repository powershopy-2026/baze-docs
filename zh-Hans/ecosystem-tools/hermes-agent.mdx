...
| --- | --- | --- | --- |
| 快速接入文本模型 | Custom endpoint | `https://baze-api.powerbuyin.top/v1` | OpenAI-compatible 文本模型 |
| 复核 Anthropic Messages 路径 | 仅在当前 Hermes 版本确认会请求 `/v1/messages` 后使用 `api_mode: anthropic_messages` | `https://baze-api.powerbuyin.top` | 智谱、MiniMax、Qwen、BytePlus DeepSeek、已复测且支持 Messages 的 Seed 模型 |
| 使用 OpenAI-compatible 优先模型 | Custom endpoint | `https://baze-api.powerbuyin.top/v1` | 上游不支持 `/v1/messages` 的 `seed-1-6-flash-250715` 等文本模型 |
...
配置后先查看 Hermes 的请求 dump 或调试日志，确认最终请求 URL 是 `https://baze-api.powerbuyin.top/v1/messages`。如果实际请求仍然落到 `/chat/completions`，请不要把该路径作为 Hermes Agent 稳定配置使用。
