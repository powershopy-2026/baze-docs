...
- OpenAI-compatible 路径：使用 `openai-completions`，base URL 为 `https://baze-api.powerbuyin.top/v1`。
- Anthropic 路径：使用 `anthropic-messages`，base URL 为 `https://baze-api.powerbuyin.top`。
...
| --- | --- | --- | --- |
| OpenAI-compatible 文本模型 | `openai-completions` | `https://baze-api.powerbuyin.top/v1` | 支持 `/v1/chat/completions` 的文本模型 |
| Claude/Anthropic 路径模型 | `anthropic-messages` | `https://baze-api.powerbuyin.top` | 智谱、MiniMax、Qwen、BytePlus DeepSeek、已复测且支持 Messages 的 Seed 模型 |
| OpenAI-compatible 优先模型 | `openai-completions` | `https://baze-api.powerbuyin.top/v1` | 上游不支持 `/v1/messages` 的 `seed-1-6-flash-250715` 等文本模型 |
