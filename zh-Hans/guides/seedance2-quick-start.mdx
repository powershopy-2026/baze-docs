...
> **Base URL：** 两种接口共用同一个 Base URL：`https://baze-api.powerbuyin.top`
...
2. **获取 API Key**：进入 [API Keys 管理页面](https://baze-api.powerbuyin.top-keys)，创建新的 Key，复制并妥善保存
...
> 两种接口共用同一个 Base URL：`https://baze-api.powerbuyin.top`，区别在于请求路径不同。
...
```bash
curl -X POST https://baze-api.powerbuyin.top/byteplus/api/v3/contents/generations/tasks \
...
```bash
curl "https://baze-api.powerbuyin.top/byteplus/api/v3/contents/generations/tasks/XXXX" \
  -H "Authorization: YOUR_API_KEY"
```
...
```bash
curl "https://baze-api.powerbuyin.top/byteplus/api/v3/contents/generations/tasks?page_num=1&page_size=20&filter.status=succeeded" \
  -H "Authorization: YOUR_API_KEY"
```
...
```bash
curl -X DELETE "https://baze-api.powerbuyin.top/byteplus/api/v3/contents/generations/tasks/XXXX" \
  -H "Authorization: YOUR_API_KEY"
```
...
|---|---|---|
| Base URL | `https://baze-api.powerbuyin.top`（相同） | `https://baze-api.powerbuyin.top`（相同） |
...
BASE_URL = "https://baze-api.powerbuyin.top"
...
> 详细的资产库使用说明请参考 [资产库快速上手](./资产库_快速上手.md)。
...
```bash
curl -X POST https://baze-api.powerbuyin.top/v1/videos \
  -H "Authorization: YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "dreamina-seedance-2-0-260128",
...
```bash
curl "https://baze-api.powerbuyin.top/v1/videos/XXXX" \
  -H "Authorization: YOUR_API_KEY"
```
...
```bash
curl "https://baze-api.powerbuyin.top/byteplus/api/v3/contents/generations/tasks?page_num=1&page_size=20" \
  -H "Authorization: YOUR_API_KEY"
```
...
```bash
curl -X POST https://baze-api.powerbuyin.top/v1/asset/upload \
  -H "Authorization: YOUR_API_KEY" \
...
```bash
curl "https://baze-api.powerbuyin.top/v1/asset/jobs/get-asset-id?task_id=XXXX"
```
