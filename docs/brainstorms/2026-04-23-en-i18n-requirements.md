...
The Mintlify docs site has a dual-language framework (`zh-Hans/` + `en/`) but the English side is incomplete. The `zh-Hans/` navigation uses the new `zmodel*` structure (Image Models, Video Models, Chat Models, Audio Models) while `en/` still references old-style pages and has empty zmodel groups.

## Requirements

### File creation

- **R1:** Create English mdx pages under `en/zmodelImage/`, `en/zmodelVideo/`, `en/zmodelChat/`, `en/zmodelAudio/` mirroring the 50 CN navigation pages. Exclude `zh-Hans/zmodelChat/ali/qwen3.5-flash.mdx` (exists on disk but absent from CN navigation). Breakdown: zmodelImage 10, zmodelVideo 12, zmodelChat 21, zmodelAudio 7.
...
- **R4:** Keep mdx frontmatter `openapi` in the full Mintlify format: `"/api-reference/en/zmodel*/<provider>/<model>.json <METHOD> <path>"` — only the directory prefix changes from `zh-Hans/` to `en/`, the HTTP method and endpoint path are preserved from the CN source
...
### Out of scope
- `zh-Hans/zmodelChat/ali/qwen3.5-flash.mdx` (orphan, not in CN navigation)
...
Each `zh-Hans/zmodelImage/<provider>/<model>.mdx` maps to `en/zmodelImage/<provider>/<model>.mdx`.
Each `zh-Hans/zmodelVideo/<provider>/<model>.mdx` maps to `en/zmodelVideo/<provider>/<model>.mdx`.
Each `zh-Hans/zmodelChat/<provider>/<model>.mdx` maps to `en/zmodelChat/<provider>/<model>.mdx` (excluding qwen3.5-flash).
Each `zh-Hans/zmodelAudio/<provider>/<model>.mdx` maps to `en/zmodelAudio/<provider>/<model>.mdx`.

Same pattern applies to `api-reference/zh-Hans/zmodel*/<provider>/<model>.json` → `api-reference/en/zmodel*/<provider>/<model>.json`.
