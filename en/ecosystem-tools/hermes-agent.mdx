...
| --- | --- | --- | --- |
| Quick text-model setup | Custom endpoint | `https://baze-api.powerbuyin.top/v1` | OpenAI-compatible text models |
| Anthropic Messages verification | Use `api_mode: anthropic_messages` only after your Hermes version is confirmed to request `/v1/messages` | `https://baze-api.powerbuyin.top` | Recommended Zhipu, MiniMax, Qwen, BytePlus DeepSeek, and re-validated Seed models that support Messages |
| OpenAI-compatible preferred models | Custom endpoint | `https://baze-api.powerbuyin.top/v1` | Text models whose upstream provider does not support `/v1/messages`, such as `seed-1-6-flash-250715` |
...
| URL | `https://baze-api.powerbuyin.top/v1` |
...
```text
https://baze-api.powerbuyin.top/v1/chat/completions
```
...
After configuring it, inspect the Hermes request dump or debug log. The final request URL should be `https://baze-api.powerbuyin.top/v1/messages`. If the request still lands on `/chat/completions`, do not treat this path as a stable Hermes Agent setup.
...
```bash
curl -X POST "https://baze-api.powerbuyin.top/v1/chat/completions" \
...
```bash
curl -X POST "https://baze-api.powerbuyin.top/v1/messages" \
  -H "Content-Type: application/json" \
